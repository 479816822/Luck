...
public class Luck {
	public static void main(String[] args) {
		...
		String[] passWord = new String[10]; // 记录每一个用户的额登陆密码
		String name = "";
		int[] Id = new int[10];
		String upPass = "";
		String upname = "";
		...
		while (true) {
			...
			case 1:
				...
				if (number < 10) {
					...
					System.out.println(name2 + "		" + passWord[number] + "		"
							+ Id[number]);
					number++;

				} else {
					System.out.println("对比起！注册的人数已满！");
				}
				break;
			case 2:
				// 登陆
				...
				while (count <= 3) {
					...
					for (j = 0; j < number; j++) {
						...
						if ((name.equals(upName[j]))
								&& (upPass.equals(passWord[j]))) {
			...
			case 3:
				// 抽奖
				int luckNumber[] = new int[5];//用来记录产生的随机数
				int ID = 0;
